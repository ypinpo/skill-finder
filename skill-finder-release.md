...
```bash
curl -s "https://api.github.com/repos/<org>/<repo>/contents/<path>/SKILL.md" | python3 -c "import sys,json,base64; d=json.load(sys.stdin); print(base64.b64decode(d['content']).decode())"
```
...
| 维度 | 检查项 |
|------|--------|
| Shell 命令 | rm -rf, curl | bash, eval, 提权命令 |
| 网络行为 | 向未知域名 POST、下载二进制 |
| 文件访问 | 读取 ~/.ssh, .env, 写入系统目录 |
